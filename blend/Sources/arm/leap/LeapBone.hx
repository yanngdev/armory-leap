...
class LeapBone {
  ...
  public function update(prevJoint:Array<Float>, nextJoint:Array<Float>, rotation:Array<Array<Float>>) {
    ...
    this.rotation.fromRotationMat(new Mat4(
          rotation[0][0] * (handType == LeapHumanHand.Left ? -1 : 1), -rotation[2][0], rotation[1][0], 0,
          -rotation[0][2] * (handType == LeapHumanHand.Left ? -1 : 1), rotation[2][2], -rotation[1][2], 0,
          rotation[0][1] * (handType == LeapHumanHand.Left ? -1 : 1), -rotation[2][1], rotation[1][1], 0,
          0, 0, 0, 0
    ));
